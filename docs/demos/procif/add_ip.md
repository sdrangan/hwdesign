...
Packaging is a GUI step — the build script covers simulation, synthesis and
timing, but stops short of packaging.

* [Launch the Vitis GUI](../../support/amd/launching.md) and open the workspace for the `scalar_fun` project
* In the **FLOW** panel (left sidebar), select **Package → Run**.  
  This wraps the synthesized RTL into a reusable IP block, complete with metadata and interface definitions.
* The packaging will have created a directory of files containing the *IP*.  It will be located under the component directory, in 
~~~bash
  scalar_fun_vitis/hls_component/simp_fun/hls/impl/ip
~~~
...
* Go to **Tools->Settings->Project Settings->IP->Repository**.  Select the `+` sign in **IP Repositories**.  Navigate to the directory with the packaged component.  In our case, this was at:  `hwdesign/demos/scalar_fun/scalar_fun_vitis/hls_component/simp_fun/hls/impl/ip`.  
* Select the `Add IP` button (`+`) again.  Add this IP.  Now the `simp_fun` block should show up as an option.  If it doesn't it is possible that you synthesized for the wrong FPGA part number.  
* You should see an Vitis IP block with ports `s_axi_CTRL`, `interrupt` and some clocks.  Select the **run block automation**.  This will connect the IP to the processing system and may add some additional blocks for resetting the processor and interfacing the IP with the processor.  
* In this simple example, we will not connect the `interrupt` signal.
* Select the Vitis IP block.  In the `Block Properties` panel, select the `General` tab, and make sure the block is named `simp_fun`.  This is the name we will use when calling the function from `PYNQ`, where it appears as `overlay.simp_fun_0`.
